...
| MS-K | P2 | RSI2MR partial-fill semantics audit — `on_fill(BUY)` does `_position_shares = int(result.filled)` (overwrite, cumulative-fill semantics) but `on_fill(SELL)` zeroes `_position_shares` unconditionally regardless of partial vs full. With bracket orders today partials are unlikely, but the strategy doesn't enforce all-or-nothing. A partial SELL would corrupt `_get_strategy_attributed_equity()`'s unrealized term (zero shares × stale entry). Pre-existing; surfaced by 2026-05-10 MS-B second-pass CR HIGH-1. |
