...
**Pre-implementation CR sub-rule.** If the user's focus request includes "unbiased review", "CR the plan", or "subagent review", spawn the review (the `review-loop` skill or the `deep-review` skill) as PART of Step 7 — before presenting the revised plan. The review is the last bullet of Step 7; the revised plan that comes out of it is what the user approves.
