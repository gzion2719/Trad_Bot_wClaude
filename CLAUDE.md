...
**Phase 6 — paper trading.** Bot running on VPS (paper account, both strategies live; MS-I + MS-C3 deployed 2026-05-11 19:22 UTC). Dashboard Phase 4 fully deployed. **Dashboard Phase 5 (per-strategy view) — Sessions 1 + 2 of 3 shipped (2026-05-13)**: Session 1 (2026-05-12) delivered three endpoints (`/api/strategies`, `/api/strategies/{name}/summary`, `/api/strategies/{name}/fills`) with path-safety + 30s TTL cache, Mission Control Recent Fills' Strategy column, and `config/strategy_metadata.py` extraction so the dashboard imports class-free metadata only. `TradeLog` gained `lifetime_summary()` + public `connection()` context manager with `busy_timeout=5000` (latent SQLite-lock fix). Session 2 (2026-05-13) shipped the user-visible Strategies top tab + dynamic secondary tab strip + per-strategy KPI strip on top of those endpoints — UI-only, N-tab keyboard refactor (circular ArrowL/R + Home/End), `sessionStorage` persistence validated against the fetched list, lazy first-activation init, 30s polling gated by `_onStratTab && document.visibilityState`, `aria-live` KPI region, sub-header showing symbol + schedule, conditional legacy-NULL-basis warning row. **DB-path bug bundled** in S2's PR: `dashboard/app.py` was reading `data/trades.db` while `main.py` writes `data/paper_trades.db` — the dashboard had been reading from an empty file on VPS the whole time. Fixed; legacy-NULL-basis surface goes live as a side effect. New `test_ds27` URL-drift tripwire parses every `fetch("/api/...")` in `dashboard.js` and asserts each maps to a registered FastAPI route — permanent protection against the 2026-05-04 `console/lock/release` typo class. 30 + 1 = 31 dashboard tests. Two unbiased CR rounds (Session 1 pre+post-impl; Session 2 pre-impl with 13 findings folded in pre-code). B-10 confirmed stable. **MS-A1+A2, MS-D, MS-B, MS-K, eager-save migration, MS-C, MS-J, MS-I, MS-C3** all shipped. State schema v2 with `partial_fill_halt` persistence.

**1 open code-review item:** CR-07 (`ib_insync` migration to `ib_async` fork — BACKLOG, multi-week).

**Immediate next steps:**
1. **Dashboard Phase 5 Session 3** — paginated history table per strategy + streaming CSV export endpoint + DB-X5 shared `TestClient` auth-failure fixture for end-to-end auth coverage on both new and existing endpoints.
2. **Spawned task — FastAPI `+inf → null` in `/summary`.** `data/trade_log.py:_round_profit_factor` returns `float('inf')` for only-wins; FastAPI's default JSONResponse silently converts to `null` so the dashboard renders `—` instead of `∞`. Renderer already accepts both `Infinity` literal and string `"Infinity"`, so server-side fix needs no JS change. Pick this one before S3 if any registered strategy starts approaching an only-wins state.
3. **MS-C2 (P2)** — IBKR `reqHistoricalData` fallback for `_refresh_history`. Design item — auto_adjust vs unadjusted-TRADES (or `ADJUSTED_LAST`) needs resolution before implementing.
