...
1. **Open + merge the develop→main ship PR** ([compare/main...develop](https://github.com/gzion2719/Trad_Bot_wClaude/compare/main...develop)) — lands Phase 0 (CODEOWNERS + gitleaks 8.30.1 + DU/DUE regex widen + HANDOFF SSH fix + drifted-content archive + review-report stubs). 10+ commits ahead of main.
2. **Verify B-13 across the next nightly auto-restart** — Sun 2026-05-24 23:59 UTC should show a clean `Market data mode: delayed` log line after the daemon-thread reconnect, no `RuntimeError`, no `Error 10089` Monday morning.
3. **F-DOC-08 — CLAUDE.md slim** (own session per the 2026-05-21 cont. 2 Step 7 self-critique) — extract incident archaeology to `docs/INCIDENTS.md`, keep Current state ≤150 lines. Preserve OPEN_SESSION_PROTOCOL.md Step 4 read-list.
4. **Phase 1 (safety floor)** — fail-fast `start_all`, `safe_place_protective_order` for bracket legs + grep tripwire, ntfy alerting on halt/CB/error storms + weekly synthetic ping.
5. **MS-C2 (P2)** — IBKR `reqHistoricalData` fallback for `_refresh_history`. MEASUREMENT-GATED — do not design or build before 2026-06-12 when `scripts/yfinance_outage_report.py` runs.
6. **GC-4 — TLS for the dashboard** (Caddy/nginx + tailscale-cert). Reprioritized into Phase 6 by IMPROVEMENT_PLAN but available as an immediate-next swap if preferred.
7. **Phase 0 backups (F-OPS-02)** — blocked on B2 bucket + app-key creation; once available, daily sqlite + state-file off-VPS push + restore validator.
