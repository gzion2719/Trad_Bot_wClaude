...
1. **Merge B-11 PRs + deploy** — `feature/ibkr-client-thread-safe-market-data` head is `554caf4` (the actual fix; `fff3950` + `b8ec0da` were already merged but not enough). Open [feature→develop](https://github.com/gzion2719/Trad_Bot_wClaude/compare/develop...feature/ibkr-client-thread-safe-market-data) then [develop→main](https://github.com/gzion2719/Trad_Bot_wClaude/compare/main...develop). VPS: `ssh chappy-vps → sudo -i → cd /opt/tradebot && git pull origin main && systemctl restart tradebot`. Verify PingPong fills via `journalctl -fu tradebot | grep -E "(PingPong|placed|FILLED|BUY|SELL)"` — first tick is 5 min after restart (`Interval(300)`). If the same error returns, do NOT assume our fix missed another layer until you read the ib_insync source for the specific failing call path — `Client.send`/`sendMsg`/`getLoop` was the gotcha this session.
2. **Open the S3c PRs + deploy** — `feature/strat-fills-csv-export` is committed (`959bb38`) and pushed but PRs not yet opened. Open feature→develop and develop→main, then `systemctl restart tradebot-dashboard`. Closes the read side of Dashboard Phase 5.
