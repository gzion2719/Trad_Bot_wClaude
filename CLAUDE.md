...
1. **Deploy B-12 to VPS + verify** — `ssh chappy-vps → sudo -i → cd /opt/tradebot && git pull origin main && systemctl restart tradebot`. Then watch `journalctl -fu tradebot | grep -E "(PingPong|placed|FILLED|BUY|SELL)"`. First tick is 5 min after restart; expect alternating BUY/SELL every 5 min during RTH, and the dashboard `PingPongTest-AAPL` Total Fills counter to actually increment now (was stuck at 1 due to M1 race). NB: any QQQ/SPY positions that appear in the IBKR Account tab but are missing from the dashboard's per-strategy view are pre-B-12 fills that M1 hid from TradeLog — they're at the broker but unrecoverable as TradeLog rows.
...
4. **GC-4 — TLS for the dashboard** (Caddy/nginx + tailscale-cert). Only unblocked roadmap build item.
