...
- **Risk rules:** the risk component sections in `CLAUDE.md` (RiskManager `plan_trade`, the 2% rule, the 1:3 R/R rule) are law. Never write code that violates them. If the user asks for something that contradicts them, stop and flag it. (TradeBot has no separate `docs/RISK_MANAGEMENT.md` — the risk law lives in `CLAUDE.md` and `risk/risk_manager.py`.)
...
**Rule:** every push runs `make pre-push` first. It is a verbatim mirror of `.github/workflows/ci.yml`:
