...
| MS-C3 | P3 | VIX feed alerting follow-ups — initial entry (2026-05-11) was based on a wrong premise; `VIXFeed.get_latest_close()` already has `_fire_stale_alert` in `data/vix_feed.py:108` that POSTs to ntfy when the fresh fetch fails AND the cache is stale (>24h) or absent, with a 24h cooldown on `_last_ntfy_at`. The real (smaller) gaps: (a) `_last_ntfy_at` is in-memory only — a restart during a multi-day outage resets the cooldown and can re-fire on every restart (mirror MS-C in-memory tradeoff; acceptable but undocumented); (b) no recovery log line when yfinance comes back — only `logger.debug` fires on cache-hit, so the operator can't see "outage cleared" in journalctl; (c) intermittent fetch failures whose retries succeed against the 5-day window go un-alerted forever (cache always fresh, never stale). Worth a 30-LOC pass after MS-C2; not urgent. |
