...
| 5.7 | [x] | P2 | Monitoring dashboard — **Phase 1** done 2026-05-02. **Phase 2** done 2026-05-02. **Phase 3** (control plane) done 2026-05-02, deployed/verified 2026-05-03. **Phase 4** (IBKR Account tab) done 2026-05-04: KPI strip (Settled Cash / Unrealized P&L / Realized P&L), "IBKR Account" tab with equity chart + positions table + balances list; `AccountSnapshotPoller` writes `data/account_snapshot.json` every 30s via file-IPC; `/api/today` + `/api/recent-fills` now require session cookie (auth uniformity); 47 new tests (AS-01..10 + DB-30..37); spec → 2× unbiased review → Sonnet impl → code review → QA → fixes applied. PRs pending deploy to VPS. |
